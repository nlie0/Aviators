...
def collect_calibration_trial(trial_index):
    """
    Collect and store one calibration trial's EEG data (no model needed).
    Waits for the photosensor edge, segments the trial, and appends to
    ...
    print(f"  Calibration trial {trial_index} collected: {trial_eeg.shape}")
...
try:
    ...
    while True:
        # In calibration mode, stop after all trials are collected
        if CALIBRATION_MODE and trial_sequence is not None:
            if trial_index >= len(trial_sequence):
                print(f"All {len(trial_sequence)} calibration trials collected!")
                break
        ...
        if MODE == "testing":
            ...
            if CALIBRATION_MODE and trial_sequence is not None:
                # Calibration: collect trial EEG data (no model needed)
                ...
                collect_calibration_trial(trial_index)
                target_lane = current_lane  # stay in place during calibration
                trial_index += 1
            else:
                # Live play: use model to predict lane
